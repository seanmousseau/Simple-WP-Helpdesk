...
function swh_render_client_portal() {
	...
	if ( swh_is_token_expired( $ticket_id ) ) {
		...
		if ( $portal_url ) {
			echo '<p class="swh-expired-recovery">' . esc_html__( 'Your link has expired. To get a new link, ', 'simple-wp-helpdesk' ) .
				'<a href="' . esc_url( $portal_url ) . '">' . esc_html__( 'look up your ticket here', 'simple-wp-helpdesk' ) . '</a>.' .
			'</p>';
		}
		echo '</div>';
		return;
	...
	<?php if ( $closed_status !== $data['status'] ) : ?>
		...
		<form method="POST" action="" enctype="multipart/form-data">
			...
			if ( 'honeypot' === $spam_method ) {
				echo '<div aria-hidden="true" style="clip-path:inset(50%);height:1px;overflow:hidden;position:absolute;white-space:nowrap;width:1px;"><label aria-hidden="true">Leave this empty</label><input type="text" name="swh_website_url_hp" value="" tabindex="-1" autocomplete="off"></div>';
		...
		<div class="swh-alert swh-alert-error" role="alert">
			...
			<form method="POST" action="" enctype="multipart/form-data">
				...
				<?php if ( 'honeypot' === $spam_method ) : ?>
					<div aria-hidden="true" style="clip-path:inset(50%);height:1px;overflow:hidden;position:absolute;white-space:nowrap;width:1px;"><label aria-hidden="true">Leave this empty</label><input type="text" name="swh_website_url_hp" value="" tabindex="-1" autocomplete="off"></div>
				<?php endif; ?>
				<div class="swh-form-group">
