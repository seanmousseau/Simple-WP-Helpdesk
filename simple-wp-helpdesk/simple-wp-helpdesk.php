		...
		<form method="POST" action="">
			...
			<div id="tab-routing" class="swh-tab-content" style="display:none;">
				<table class="form-table">
					...
					foreach ( $techs as $t ) :
						?>
						<option value="<?php echo esc_attr( $t->ID ); ?>" <?php selected( get_option( 'swh_default_assignee' ), $t->ID ); ?>><?php echo esc_html( $t->display_name ); ?></option><?php endforeach; ?></select></td></tr>
...
function swh_status_meta_box_html( $post ) {
	...
	<select name="ticket_assigned_to" style="width: 100%; margin-bottom: 10px;">
		...
		foreach ( $techs as $t ) :
			?>
			<option value="<?php echo esc_attr( $t->ID ); ?>" <?php selected( $assignee, $t->ID ); ?>><?php echo esc_html( $t->display_name ); ?></option><?php endforeach; ?>
	</select>
