...
function swh_run_upgrade_routine() {
    ...
    if ( version_compare( $db_version, SWH_VERSION, '<' ) ) {
        ...
        foreach ( $defs as $key => $val ) { add_option( $key, $val ); }
...
function swh_uninstall() {
    if ( get_option('swh_delete_on_uninstall') === 'yes' ) {
        $tickets = get_posts( array('post_type' => 'helpdesk_ticket', 'numberposts' => -1, 'post_status' => 'any') );
        foreach($tickets as $t) { swh_delete_ticket_and_files($t->ID); }
        remove_role('technician');
        foreach(swh_get_all_option_keys() as $opt) { delete_option($opt); }
...
function swh_register_ticket_cpt() {
    register_post_type( 'helpdesk_ticket', array(
        'labels'      => array( 'name' => 'Tickets', 'singular_name' => 'Ticket', 'add_new_item' => 'Add New Ticket', 'edit_item' => 'Edit Ticket' ),
        'public'      => false,  
        'show_ui'     => true,   
...
function swh_add_enctype_to_post_form() {
    global $post;
    if ( $post && $post->post_type === 'helpdesk_ticket' ) echo ' enctype="multipart/form-data"';
}
...
function swh_load_spam_scripts_in_head() {
    ...
    if ( $spam_method === 'recaptcha' ) {
        echo '<script src="https://www.google.com/recaptcha/api.js" async defer></script>' . "\n";
    } elseif ( $spam_method === 'turnstile' ) {
...
function swh_get_defaults() {
    ...
    if ( $defaults === null ) {
        $defaults = array(
            'swh_ticket_priorities' => 'Low, Medium, High', 'swh_default_priority' => 'Medium', 
            'swh_ticket_statuses' => 'Open, In Progress, Resolved, Closed', 'swh_default_status' => 'Open', 
            'swh_resolved_status' => 'Resolved', 'swh_closed_status' => 'Closed', 'swh_reopened_status' => 'Open',
...
function swh_get_all_option_keys() {
    return array_merge(array_keys(swh_get_defaults()), array(
        'swh_default_assignee', 'swh_fallback_email', 'swh_max_upload_size', 'swh_autoclose_days',
        'swh_retention_tickets_days', 'swh_retention_attachments_days',
        'swh_spam_method', 'swh_recaptcha_site_key', 'swh_recaptcha_secret_key', 'swh_turnstile_site_key', 'swh_turnstile_secret_key',
        'swh_db_version'
    ));
}

function swh_get_statuses() { $defs = swh_get_defaults(); return array_map( 'trim', explode( ',', get_option( 'swh_ticket_statuses', $defs['swh_ticket_statuses'] ) ) ); }
function swh_get_priorities() { $defs = swh_get_defaults(); return array_map( 'trim', explode( ',', get_option( 'swh_ticket_priorities', $defs['swh_ticket_priorities'] ) ) ); }
...
function swh_get_secure_ticket_link( $ticket_id ) {
    ...
    if ( $base_url && $token ) return add_query_arg( array( 'swh_ticket' => $ticket_id, 'token' => $token ), $base_url );
    return false;
}
...
function swh_get_admin_email( $ticket_id = 0 ) {
    if ( $ticket_id ) {
        ...
        if ( $assigned ) { $user = get_userdata( $assigned ); if ( $user ) return $user->user_email; }
    }
    $default_assignee = get_option( 'swh_default_assignee' );
    if ( $default_assignee ) { $user = get_userdata( $default_assignee ); if ( $user ) return $user->user_email; }
...
function swh_normalize_files_array($files) {
    ...
    if(isset($files['name']) && is_array($files['name'])) {
        foreach($files['name'] as $key => $name) {
            if ($name) {
                $normalized[] = array( 'name' => $files['name'][$key], 'type' => $files['type'][$key], 'tmp_name' => $files['tmp_name'][$key], 'error' => $files['error'][$key], 'size' => $files['size'][$key] );
            }
        }
    }
    return $normalized;
...
function swh_handle_multiple_uploads( $file_array ) {
    ...
    $allowed_mimes = array( 'jpg|jpeg|jpe' => 'image/jpeg', 'gif' => 'image/gif', 'png' => 'image/png', 'pdf' => 'application/pdf', 'doc' => 'application/msword', 'docx' => 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'txt' => 'text/plain' );
...
function swh_delete_ticket_and_files( $ticket_id ) {
    ...
    if (!empty($main_atts) && is_array($main_atts)) {
        foreach ($main_atts as $url) { swh_delete_file_by_url($url); }
    }
    
    $legacy_id = get_post_meta($ticket_id, '_ticket_attachment_id', true);
    if ($legacy_id) { wp_delete_attachment($legacy_id, true); }
    $legacy_url = get_post_meta($ticket_id, '_ticket_attachment_url', true);
    if ($legacy_url) { swh_delete_file_by_url($legacy_url); }
    ...
    foreach ($comments as $c) {
        ...
        if (!empty($c_atts) && is_array($c_atts)) {
            foreach ($c_atts as $url) { swh_delete_file_by_url($url); }
        }
        $legacy_c_url = get_comment_meta($c->comment_ID, '_attachment_url', true);
        if ($legacy_c_url) { swh_delete_file_by_url($legacy_c_url); }
    }
...
function swh_process_autoclose() {
    ...
    $tickets = get_posts(array(
        ...
        'meta_query' => array(
            ...
            array('key' => '_resolved_timestamp', 'value' => $threshold, 'compare' => '<=', 'type' => 'NUMERIC')
        )
    ));
    ...
    foreach($tickets as $ticket) {
        ...
        $comment_id = wp_insert_comment( array(
            'comment_post_ID' => $ticket->ID, 'comment_author' => 'System Auto-Close',
            'comment_content' => "Ticket automatically closed due to inactivity ($days days).", 'comment_approved' => 1
        ));
...
function swh_process_retention_attachments() {
    ...
    $tickets = get_posts(array(
        ...
        'meta_query' => array( array('key' => '_ticket_attachments', 'compare' => 'EXISTS') )
    ));

    foreach($tickets as $ticket) {
        $atts = get_post_meta($ticket->ID, '_ticket_attachments', true);
        if (!empty($atts) && is_array($atts)) {
            foreach($atts as $url) { swh_delete_file_by_url($url); }
            delete_post_meta($ticket->ID, '_ticket_attachments'); 
            
            $comment_id = wp_insert_comment( array(
                'comment_post_ID' => $ticket->ID, 'comment_author' => 'System Maintenance',
                'comment_content' => "Original ticket attachments automatically purged (older than $days days).", 'comment_approved' => 1
            ));
            update_comment_meta($comment_id, '_is_internal_note', '1');
        }
    }
    ...
    $comments = get_comments(array(
        ...
        'number' => 1,
        'date_query' => array( array('before' => $threshold_date) ),
        'meta_query' => array( array('key' => '_attachments', 'compare' => 'EXISTS') )
    ));
    ...
    foreach($comments as $comment) {
        ...
        if (!empty($atts) && is_array($atts)) {
            ...
            wp_update_comment(array('comment_ID' => $comment->comment_ID, 'comment_content' => $new_content));
        }
    }
}
...
function swh_register_settings_page() { add_submenu_page( 'edit.php?post_type=helpdesk_ticket', 'Helpdesk Settings', 'Settings', 'manage_options', 'swh-settings', 'swh_render_settings_page' ); }
...
function swh_render_settings_page() {
    ...
    if ( isset($_POST['swh_gdpr_delete']) && wp_verify_nonce($_POST['swh_danger_nonce'], 'swh_danger_action') ) {
        $gdpr_email = sanitize_email($_POST['swh_gdpr_email']);
        if ( $gdpr_email ) {
            $tickets = get_posts( array('post_type' => 'helpdesk_ticket', 'numberposts' => -1, 'post_status' => 'any', 'meta_query' => array( array('key' => '_ticket_email', 'value' => $gdpr_email) )) );
            $count = count($tickets);
            foreach($tickets as $t) { swh_delete_ticket_and_files($t->ID); }
            echo '<div class="updated error"><p><strong>Successfully deleted ' . $count . ' ticket(s) and all associated files for ' . esc_html($gdpr_email) . '.</strong></p></div>';
        } else {
    ...
    // MASS EXECUTIONS
    if ( isset($_POST['swh_purge_tickets']) && wp_verify_nonce($_POST['swh_danger_nonce'], 'swh_danger_action') ) {
        $tickets = get_posts( array('post_type' => 'helpdesk_ticket', 'numberposts' => -1, 'post_status' => 'any') );
        foreach($tickets as $t) { swh_delete_ticket_and_files($t->ID); }
        echo '<div class="updated error"><p><strong>All tickets & files have been successfully purged.</strong></p></div>';
    }
    if ( isset($_POST['swh_factory_reset']) && wp_verify_nonce($_POST['swh_danger_nonce'], 'swh_danger_action') ) {
        $tickets = get_posts( array('post_type' => 'helpdesk_ticket', 'numberposts' => -1, 'post_status' => 'any') );
        foreach($tickets as $t) { swh_delete_ticket_and_files($t->ID); }
        foreach($options_list as $opt) { delete_option($opt); }
        delete_option('swh_delete_on_uninstall');
        delete_option('swh_db_version');
    ...
    // SAVE SETTINGS
    if ( isset( $_POST['swh_save_settings'] ) && wp_verify_nonce( $_POST['swh_settings_nonce'], 'swh_save_settings_action' ) ) {
    ...
    function swh_field($name, $defs, $type='text') {
        ...
        if ($type=='textarea') {
            echo '<textarea name="'.$name.'" rows="4" class="large-text" data-default="'.esc_attr($defs[$name]).'">'.esc_textarea($val).'</textarea>';
        } else {
            echo '<input type="text" name="'.$name.'" value="'.esc_attr($val).'" class="regular-text" style="width:100%; max-width:500px;" data-default="'.esc_attr($defs[$name]).'">';
        }
            ...
            <div id="tab-routing" class="swh-tab-content" style="display:none;">
                <table class="form-table">
                    <tr><th scope="row">Default Assignee</th>
                        <td><select name="swh_default_assignee"><option value="">-- Unassigned --</option><?php foreach($techs as $t): ?><option value="<?php echo $t->ID; ?>" <?php selected(get_option('swh_default_assignee'), $t->ID); ?>><?php echo esc_html($t->display_name); ?></option><?php endforeach; ?></select></td></tr>
...
function swh_status_meta_box_html( $post ) {
    ...
    if ( $status && !in_array($status, $statuses) ) $statuses[] = $status;
    if ( $priority && !in_array($priority, $priorities) ) $priorities[] = $priority;
    ...
    if ( !empty($all_attachments) ) : ?>
        ...
        <?php foreach($all_attachments as $i => $url): ?>
            <a href="<?php echo esc_url($url); ?>" target="_blank" class="button button-secondary button-small" style="margin-top:5px; margin-right:5px;">File <?php echo $i+1; ?></a>
        <?php endforeach; ?></p>
    <?php endif; ?>
    ...
    <select name="ticket_assigned_to" style="width: 100%; margin-bottom: 10px;">
        ...
        <?php foreach($techs as $t): ?><option value="<?php echo $t->ID; ?>" <?php selected( $assignee, $t->ID ); ?>><?php echo esc_html($t->display_name); ?></option><?php endforeach; ?>
    </select>
    
    <p><strong>Priority:</strong></p>
    <select name="ticket_priority" style="width: 100%; margin-bottom: 10px;">
        <?php foreach($priorities as $p): ?><option value="<?php echo esc_attr($p); ?>" <?php selected( $priority, $p ); ?>><?php echo esc_html($p); ?></option><?php endforeach; ?>
    </select>

    <p><strong>Status:</strong></p>
    <select name="ticket_status" style="width: 100%;">
        <?php foreach($statuses as $s): ?><option value="<?php echo esc_attr($s); ?>" <?php selected( $status, $s ); ?>><?php echo esc_html($s); ?></option><?php endforeach; ?>
    </select>
    <?php
}
    ...
    if ( $comments ) {
        foreach ( $comments as $comment ) {
            ...
            if ( $is_internal ) {
                ...
                $bg_color = '#fff3cd'; $border = '#ffeeba';
            } else {
                $author_name = $is_user ? 'Client (' . esc_html($comment->comment_author) . ')' : 'Technician (' . esc_html($comment->comment_author) . ')';
                $bg_color = $is_user ? '#f9f9f9' : '#e6f7ff'; $border = '#0073aa';
            }

            echo '<div style="background: ' . $bg_color . '; padding: 10px 15px; margin-bottom: 10px; border-left: 4px solid ' . $border . '; border-radius: 3px;">';
            ...
            if ( !empty($attachments) && is_array($attachments) ) {
                ...
                foreach($attachments as $i => $url) {
                    echo '<a href="' . esc_url($url) . '" target="_blank" class="button button-small" style="margin-right:5px;">Attachment '.($i+1).'</a>';
                }
                echo '</div>';
            }
            echo '</div>';
...
function swh_save_ticket_data( $post_id, $post, $update ) {
    ...
    if ( ! isset( $_POST['swh_ticket_nonce'] ) || ! wp_verify_nonce( $_POST['swh_ticket_nonce'], 'swh_save_ticket' ) ) return;
    ...
    $assigned_to = sanitize_text_field( $_POST['ticket_assigned_to'] );
    ...
    $resolved_status = get_option('swh_resolved_status', $defs['swh_resolved_status']);
    if ($new_status === $resolved_status && $old_status !== $new_status) {
        update_post_meta($post_id, '_resolved_timestamp', current_time('timestamp'));
    } elseif ($old_status === $resolved_status && $new_status !== $resolved_status) {
    ...
    $reply_text = sanitize_textarea_field( $_POST['swh_tech_reply_text'] ?? '' );
    ...
    if ( $reply_text || $has_files ) {
        ...
        $comment_id = wp_insert_comment( array( 'comment_post_ID' => $post_id, 'comment_author' => $current_user->display_name, 'comment_author_email' => $current_user->user_email, 'comment_content' => $comment_content, 'comment_approved' => 1 ) );
        update_comment_meta( $comment_id, '_is_user_reply', '0' );

        $attach_urls = swh_handle_multiple_uploads( $_FILES['swh_tech_reply_attachments'] );
        if ( !empty($attach_urls) ) update_comment_meta( $comment_id, '_attachments', $attach_urls );

        $data['message'] = $reply_text ?: 'Attached file(s)';
    }

    $status_changed = ($update && $old_status !== $new_status);
    $is_resolving   = ($new_status === $resolved_status);
...
function swh_ticket_frontend() {
    ...
    if ( isset( $_GET['swh_ticket'] ) && isset( $_GET['token'] ) ) {
        ...
        if ( isset( $_POST['swh_user_close_ticket_submit'] ) && wp_verify_nonce( $_POST['swh_close_nonce'], 'swh_user_close' ) ) {
            update_post_meta( $ticket_id, '_ticket_status', $closed_status );
            delete_post_meta( $ticket_id, '_resolved_timestamp' ); 
            
            $comment_id = wp_insert_comment( array( 'comment_post_ID' => $ticket_id, 'comment_author' => $data['name'], 'comment_author_email' => $data['email'], 'comment_content' => "TICKET CLOSED BY CLIENT", 'comment_approved' => 1 ) );
        ...
        if ( isset( $_POST['swh_user_reopen_submit'] ) && wp_verify_nonce( $_POST['swh_reopen_nonce'], 'swh_user_reopen' ) ) {
            $reply_text = sanitize_textarea_field( $_POST['ticket_reopen_text'] ?? '' );
            ...
            if ( $reply_text || $has_files ) {
                ...
                $comment_content = $reply_text ?: 'Attached file(s)';
                $comment_id = wp_insert_comment( array( 'comment_post_ID' => $ticket_id, 'comment_author' => $data['name'], 'comment_author_email' => $data['email'], 'comment_content' => "TICKET RE-OPENED: \n" . $comment_content, 'comment_approved' => 1 ) );
                update_comment_meta( $comment_id, '_is_user_reply', '1' );
        ...
        if ( isset( $_POST['swh_user_reply_submit'] ) && wp_verify_nonce( $_POST['swh_reply_nonce'], 'swh_user_reply' ) ) {
            $reply_text = sanitize_textarea_field( $_POST['ticket_reply_text'] ?? '' );
            $has_files  = !empty($_FILES['swh_user_reply_attachments']['name'][0]);

            if ( $reply_text || $has_files ) {
                if ($data['status'] === $resolved_status) {
                    $data['status'] = $reopened_status;
                    update_post_meta( $ticket_id, '_ticket_status', $reopened_status );
                    delete_post_meta( $ticket_id, '_resolved_timestamp' ); 
                }
                ...
                $comment_content = $reply_text ?: 'Attached file(s)';
                $comment_id = wp_insert_comment( array( 'comment_post_ID' => $ticket_id, 'comment_author' => $data['name'], 'comment_author_email' => $data['email'], 'comment_content' => $comment_content, 'comment_approved' => 1 ) );
                update_comment_meta( $comment_id, '_is_user_reply', '1' );
        ...
        <div style="margin-bottom: 20px;">
            ...
            if ( $comments ) {
                foreach ( $comments as $comment ) {
                    if ( get_comment_meta( $comment->comment_ID, '_is_internal_note', true ) ) continue;
    ...
    if ( isset( $_POST['swh_submit_ticket'] ) && wp_verify_nonce( $_POST['swh_ticket_nonce'], 'swh_create_ticket' ) ) {
        
        $data = array(
            'name'       => sanitize_text_field( $_POST['ticket_name'] ),
            'email'      => sanitize_email( $_POST['ticket_email'] ),
            'title'      => sanitize_text_field( $_POST['ticket_title'] ),
            'message'    => sanitize_textarea_field( $_POST['ticket_desc'] ),
            'priority'   => sanitize_text_field( $_POST['ticket_priority'] ),
            'status'     => $default_status,
        );

        $is_spam = false;
        if ( $spam_method === 'honeypot' && !empty($_POST['swh_website_url_hp']) ) { $is_spam = true; }
        elseif ( $spam_method === 'recaptcha' ) {
            $resp = wp_remote_post( 'https://www.google.com/recaptcha/api/siteverify', array( 
                'body' => array( 'secret' => get_option('swh_recaptcha_secret_key'), 'response' => $_POST['g-recaptcha-response'] ),
                'timeout' => 10 
            ) );
            $json = json_decode( wp_remote_retrieve_body( $resp ) );
            if ( empty($json->success) ) $is_spam = true;
        } elseif ( $spam_method === 'turnstile' ) {
            $resp = wp_remote_post( 'https://challenges.cloudflare.com/turnstile/v0/siteverify', array( 
                'body' => array( 'secret' => get_option('swh_turnstile_secret_key'), 'response' => $_POST['cf-turnstile-response'] ),
                'timeout' => 10
            ) );
            $json = json_decode( wp_remote_retrieve_body( $resp ) );
            if ( empty($json->success) ) $is_spam = true;
        ...
        if ( $is_spam ) {
            ...
            if ( $ticket_id ) {
                ...
                if ( !empty($attach_urls) ) update_post_meta( $ticket_id, '_ticket_attachments', $attach_urls );

                $token = wp_generate_password( 20, false ); 
                $data['ticket_id']  = 'TKT-' . str_pad($ticket_id, 4, '0', STR_PAD_LEFT);
                $data['ticket_url'] = swh_get_secure_ticket_link( $ticket_id ) ?: add_query_arg( array( 'swh_ticket' => $ticket_id, 'token' => $token ), get_permalink() );
                ...
                $default_assignee = get_option('swh_default_assignee');
                if ( $default_assignee ) update_post_meta( $ticket_id, '_ticket_assigned_to', $default_assignee );
        ...
        if ( empty($ticket_id) ) {
            $form_name  = isset($_POST['ticket_name']) ? sanitize_text_field($_POST['ticket_name']) : $form_name;
            $form_email = isset($_POST['ticket_email']) ? sanitize_email($_POST['ticket_email']) : $form_email;
            $form_prio  = isset($_POST['ticket_priority']) ? sanitize_text_field($_POST['ticket_priority']) : $form_prio;
            $form_title = isset($_POST['ticket_title']) ? sanitize_text_field($_POST['ticket_title']) : '';
            $form_desc  = isset($_POST['ticket_desc']) ? sanitize_textarea_field($_POST['ticket_desc']) : '';
        }
    }
