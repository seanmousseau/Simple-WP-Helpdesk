...
function swh_add_enctype_to_post_form() {
	global $post;
	if ( $post && $post->post_type === 'helpdesk_ticket' ) {
			...
			<div id="tab-routing" class="swh-tab-content" style="display:none;">
				<table class="form-table">
					<tr><th scope="row">Default Assignee</th>
						...
						foreach ( $techs as $t ) :
							?>
							<option value="<?php echo $t->ID; ?>" <?php selected( get_option( 'swh_default_assignee' ), $t->ID ); ?>><?php echo esc_html( $t->display_name ); ?></option><?php endforeach; ?></select></td></tr>
...
function swh_status_meta_box_html( $post ) {
	...
	if ( $priority && ! in_array( $priority, $priorities ) ) {
		$priorities[] = $priority;
	}
	...
	<select name="ticket_assigned_to" style="width: 100%; margin-bottom: 10px;">
		...
		foreach ( $techs as $t ) :
			?>
			<option value="<?php echo $t->ID; ?>" <?php selected( $assignee, $t->ID ); ?>><?php echo esc_html( $t->display_name ); ?></option><?php endforeach; ?>
	</select>
	...
	<select name="ticket_priority" style="width: 100%; margin-bottom: 10px;">
		...
		foreach ( $priorities as $p ) :
			?>
			<option value="<?php echo esc_attr( $p ); ?>" <?php selected( $priority, $p ); ?>><?php echo esc_html( $p ); ?></option><?php endforeach; ?>
	</select>
	...
	<select name="ticket_status" style="width: 100%;">
		...
		foreach ( $statuses as $s ) :
			?>
			<option value="<?php echo esc_attr( $s ); ?>" <?php selected( $status, $s ); ?>><?php echo esc_html( $s ); ?></option><?php endforeach; ?>
	</select>
	<?php
}
...
function swh_save_ticket_data( $post_id, $post, $update ) {
	...
	if ( ! isset( $_POST['swh_ticket_nonce'] ) || ! wp_verify_nonce( $_POST['swh_ticket_nonce'], 'swh_save_ticket' ) ) {
		return;
	}
	...
	if ( ! empty( $_POST['swh_tech_note_text'] ) ) {
		$comment_id = wp_insert_comment(
			array(
				...
				'comment_content'      => sanitize_textarea_field( $_POST['swh_tech_note_text'] ),
				'comment_approved'     => 1,
			)
		);
		update_comment_meta( $comment_id, '_is_internal_note', '1' );
...
function swh_ticket_frontend() {
	...
	if ( isset( $_GET['swh_ticket'] ) && isset( $_GET['token'] ) ) {
		...
		<div class="swh-card">
			...
			if ( ! empty( $attachments ) && is_array( $attachments ) ) {
				...
				foreach ( $attachments as $i => $url ) {
					echo '<a href="' . esc_url( $url ) . '" target="_blank" style="text-decoration: underline; margin-right:10px; color:#0073aa;">File ' . ( $i + 1 ) . '</a>'; }
				echo '</p>';
			}
			?>
		</div>
				...
				foreach ( $comments as $comment ) {
					...
					if ( ! empty( $attach_urls ) && is_array( $attach_urls ) ) {
						...
						foreach ( $attach_urls as $i => $url ) {
							echo '<a href="' . esc_url( $url ) . '" target="_blank" style="text-decoration: underline; margin-right:10px; color:#0073aa; font-size:13px;">Attachment ' . ( $i + 1 ) . '</a>';
						}
						echo '</div>';
					}
					echo '</div>';
	...
	if ( isset( $_POST['swh_submit_ticket'] ) && wp_verify_nonce( $_POST['swh_ticket_nonce'], 'swh_create_ticket' ) ) {
		...
		$is_spam = false;
		if ( $spam_method === 'honeypot' && ! empty( $_POST['swh_website_url_hp'] ) ) {
			$is_spam = true; } elseif ( $spam_method === 'recaptcha' ) {
			$resp = wp_remote_post(
